...
public class AgendaHospital {
	...
	private void recuperarCitas() throws IOException, FileNotFoundException {
		...
		while ((citaAct = fichero.readLine()) != null) {
			...
			if (partesCita.length == 13) {
				Calendar fi = new GregorianCalendar(Integer.parseInt(partesCita[2]), Integer.parseInt(partesCita[3]),
						Integer.parseInt(partesCita[4]), Integer.parseInt(partesCita[5]),
						Integer.parseInt(partesCita[6]));
				Calendar ff = new GregorianCalendar(Integer.parseInt(partesCita[7]), Integer.parseInt(partesCita[8]),
						Integer.parseInt(partesCita[9]), Integer.parseInt(partesCita[10]),
	...
	public boolean generarCita(String CCPaciente, String CCMedico, int[] fechaInicio, int[] fechaFinal)
			throws FormatoFechaInvalida, IOException, FileNotFoundException, NoHayDisponibilidadCita {
		// Es necesario comprobar si el medico y el paciente existen en el hospital y
		// devolver un true or false
		// int[] fechaI : int Iano, int Imes, int dia, int Ihora, int Iminuto
		// Es neceario comprobar ademas que la fecha a agregar no tenga conflicto con
		// las fechas del medico
	...
	// Con el mismo medico
	public boolean modificarCita(String idCita, Date fechaInicio, Date fechaFinal) throws FormatoFechaInvalida,
			CitaNoExiste, PersonaNoCitas, IOException, FileNotFoundException, NoHayDisponibilidadCita, MedicoNoCitas {
		// 1. Comprobar fechas ¿Son despues de la fecha actual y la inicio está
		// despues de la final?
	...
	public boolean modificarCita(String idCita, String CCMedico, Date fechaInicio, Date fechaFinal)
			throws IOException, FileNotFoundException, FormatoFechaInvalida, NoHayDisponibilidadCita, CitaNoExiste {
		Cita ctModificar = this.buscarCitaId(idCita);

		try {
			if (this.disponibilidadCita(CCMedico, fechaInicio, fechaFinal)
					&& this.congruenciaFechas(fechaInicio, fechaFinal)) {
				// Modificamos la cita
				int indexCita = this.buscarCitaIdIndex(idCita);
				this.citas.get(indexCita).setFechaInicio(fechaInicio);
				this.citas.get(indexCita).setFechaFinal(fechaFinal);
	...
	public Cita[] ordenarCitas(ArrayList<Cita> citasN) {
		for (int i = 0; i < citasN.size(); i++) {
			for (int j = 0; j < citasN.size() - i - 1; j++) {
				...
				if ((c1.getFechaInicio().after(c2.getFechaFinal())
						|| c1.getFechaInicio().getTime() - c2.getFechaFinal().getTime() == 0)
						&& c1.getFechaFinal().after(c2.getFechaFinal())) { // Si fecha inicio está despues de siguiente
																			// fecha final, intercambiamos posiciones
					citasN.set(j + 1, c1);
					citasN.set(j, c2);
				}
			}
	...
	public boolean disponibilidadCitaExtremos(Cita[] citasN, Date fi, Date ff) {
		...
		if (citasN.length > 0) {
			boolean condicionInicio = (fi.before(citasN[0].getFechaInicio())) && (ff.before(citasN[0].getFechaInicio())
					|| ff.getTime() - citasN[0].getFechaInicio().getTime() == 0);
			boolean condicionFinal = (fi.after(citasN[citasN.length - 1].getFechaFinal())
					|| fi.getTime() - citasN[citasN.length - 1].getFechaFinal().getTime() == 0)
					&& (ff.after(citasN[citasN.length - 1].getFechaFinal()));
	...
	public boolean disponibilidadCitaHuecos(Cita[] citasN, Date fi, Date ff) {
		...
		for (int i = 0; i < citasN.length - 1; i++) {
			if ((fi.after(citasN[i].getFechaFinal()) || fi.getTime() - citasN[i].getFechaFinal().getTime() == 0) && // Está
																													// despues
																													// o
																													// es
																													// justo
																													...
																													// fecha
																													// final
					(ff.before(citasN[i + 1].getFechaInicio())
							|| ff.getTime() - citasN[i + 1].getFechaInicio().getTime() == 0)) {
				hayHueco = true;
				break;
			}
		}
	...
	public boolean disponibilidadCita(String CCMedico, Date fi, Date ff)
			throws FormatoFechaInvalida, NoHayDisponibilidadCita, MedicoNoCitas {
		...
		if (this.disponibilidadCitaExtremos(citasMedico, fi, ff)
				|| this.disponibilidadCitaHuecos(citasMedico, fi, ff)) {
			return true;
		} else {
			throw new NoHayDisponibilidadCita();
		}
